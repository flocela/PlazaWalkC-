...
TEST_CASE("Should return next positions, which are: up, right, and left.")
{
...
TEST_CASE("PositionManager_Up::atEnd() returns true if box is above the final position.")
{
    PositionManager_Up upPositionManager(1, 0, 10, 0, 10);
    REQUIRE(true == upPositionManager.atEnd(Position{5, 0}));
}

TEST_CASE("PositionManager_Up::atEnd() returns false if box is below its final position.")
{
    PositionManager_Up upPositionManager(1, 0, 10, 0, 10);
...
TEST_CASE("PositionManager_Up:: getFuturePositions doesn't return any positions that are off the board. Current position is at the right edge. It does not return a position with an x of 20, that would be off the board")
{
    // final y is at y = 0. Board is a 20x20 board.
    PositionManager_Up upPositionManager(0, 0, 19, 0, 19);
...
TEST_CASE("PositionManager_Up:: getFuturePositions doesn't return any positions that are off the board. Current position is at the left edge. It does not return a position with an x less than zero, that would be off the board.")
{
    // final y is at y = 0. Board is a 20x20 board.
    PositionManager_Up upPositionManager(0, 0, 19, 0, 19);
