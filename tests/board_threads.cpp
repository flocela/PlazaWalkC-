...
TEST_CASE("Two boxes collide (one box tries to enter the position of another box). Each box's level will increase by one. The state of the boxes will never be sent having one box's level increased while the other box's level is  yet to be increased. Before changes are sent in sendChanges() both boxes' levels will have been increased.")
{
...
TEST_CASE("Drops sent to BoardListeners are changed properly. Both their MovementType and BoxId have been changed.")
{
