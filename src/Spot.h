...
class Spot
{
    ...
    // Returns the current Box id and the SpotType. If there is not Box, then returns SpotType::left and  -1 for the Box id. Note getBoardNote() is not thread safe. While this method is being called, the Spot may be changing, so the returned BoardNote may be incomplete or invalid.
    BoardNote getBoardNote() const;

    // Updates the Spot with the BoardNote's Box id and SpotType.
    ...
    // If the Spot has a boxId of -1 and SpotType::left, then changeNote() will only be successful if the received BoardNote has SpotType::to_arrive. Any positive Box id is allowed. Any other SpotType will result in an exception.
